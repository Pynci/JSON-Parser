...
; è una funzione di supporto che permette a jsonparse di convertire le stringhe
; in ingresso (passate nella forma "\"stringa\"") in semplici stringhe lisp
; (espresse quindi nella forma "stringa").
                  ...
                  (if (equal (subseq resto 1 2) "-")
                      (let ((esponente (leggi-numero (subseq resto 2))))
                        (if (not (null (find #\. numlet)))
                          (cons (* (parse-float numlet) (expt 10 (parse-integer (concatenate 'string "-" esponente)))) (subseq resto (+ 2 (length esponente))))
                          (cons (* (parse-integer numlet) (expt 10 (parse-integer (concatenate 'string "-" esponente)))) (subseq resto (+ 2 (length esponente))))))
                      (let ((esponente (leggi-numero (subseq resto 1))))
                        (if (not (null (find #\. numlet)))
                          (cons (* (parse-float numlet) (expt 10 (parse-integer esponente))) (subseq resto (+ 1 (length esponente))))
                          (cons (* (parse-integer numlet) (expt 10 (parse-integer esponente))) (subseq resto (+ 1 (length esponente)))))))
                  (if (not (null (find #\. numlet)))
...
;;; Funzione jsonaccess
; consulta la struttura creata dalla funzione jsonparse secondo quanto indicato
; dagli eventuali argomenti passati (il primo argomento è la struttura, dal
; secondo argomento in poi bisogna specificare la lista di campi JSON da
; consultare), per poi restituire il dato individuato.
...
; caratteri facenti parte di una stringa JSON e il carattere \" di terminazione
; della stringa JSON.
...
; contenuto di un oggetto JSON e di creare la struttura lisp atta ad ospitarlo.
