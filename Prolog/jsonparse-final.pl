...
%%% inverti/2
% è un predicato di supporto che serve ricavare i dati dagli oggetti JSON
%(creati da jsonparse/2) per comporre una stringa scritta in standard JSON
...
inverti(jsonobj([','(Chiave, Valore)]), Risultato) :-
    ...
    !,
    atomic_list_concat(['{', ' ', '"', AtomoChiave, '"', ' ', ':', ' ',
			ValoreInvertito, ' ', '}'],
		       Risultato).
...
inverti(jsonobj([','(Chiave, Valore) | Altro]), Risultato) :-
    ...
    atomic_list_concat(['{', ' ', '"', AtomoChiave, '"', ' ', ':', ' ',
			ValoreInvertito, ',', AtomoCaratteriRimanenti],
		       Risultato).
...
inverti(jsonarray([Valore | Altro]), Risultato) :-
    ...
    atom_chars(AtomoCaratteriRimanenti, CaratteriRimanenti),
    atomic_list_concat(['[', ValoreInvertito, ',', ' ',
			AtomoCaratteriRimanenti],
		       Risultato).
...
arricchisci_stringa([Carattere1 | AltriCaratteri],
		    ['\\', Carattere1 | Altro]) :-
...
leggi_stringa([BackSlash, Carattere | Altro],
	      [Carattere | LettiPrecedentemente],
	      Resto) :-
...
parser_n([Cifra | Cifre],
    ...
    is_digit(Cifra),
    !,
    parser_n(Cifre, [Cifra | CifreAccumulate],
	     NumeroOttenuto, ListaCifre, CaratteriRestanti).

parser_n([Carattere | CaratteriRestanti],
	 CifreLette,
	 NumeroOttenuto,
	 ListaCifre,
...
parser_z(Caratteri, NumeroOttenuto, CaratteriRestanti) :-
    parser_z(Caratteri, [], NumeroOttenuto,
	     _CifreDelNumero, CaratteriRestanti).
...
parser_z([Meno | CifreRimanenti],
    ...
    is_meno(Meno),
    !,
    parser_z(CifreRimanenti, [Meno],
	     NumeroOttenuto, CifreDelNumero, CaratteriRestanti).
...
parser_z([Cifra | CifreRimanenti],
    ...
    is_digit(Cifra),
    !,
    parser_z(CifreRimanenti, [Cifra | CifreLette],
	     NumeroOttenuto, CifreDelNumero, CaratteriRestanti).

parser_z([Carattere | CaratteriRestanti],
	 CifreLette, NumeroOttenuto,
	 CifreDelNumero,
	 [Carattere | CaratteriRestanti]) :-
    !,
...
parser_q(Sequenza, NumeroOttenuto, CaratteriRimanenti) :-
    parser_q(Sequenza, [], NumeroOttenuto,
	     _SequenzaLetta, CaratteriRimanenti).
...
parser_true([Carattere1, Carattere2, Carattere3, Carattere4 | Resto],
    ...
    is_u(Carattere3),
    is_e(Carattere4),
    atomic_list_concat([Carattere1, Carattere2, Carattere3, Carattere4],
		       True).
